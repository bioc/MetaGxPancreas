#' Function to load Pancreatic cancer SummarizedExperiment objectsfrom the Experiment Hub
#'
#' This function returns pancreatic cancer datasets from the hub and a vector of patients from the datasets that are duplicates based on a spearman correlation > 0.98
...
loadPancreasDatasets = function(removeSeqSubset = TRUE, rescale = FALSE, minNumberGenes = 0,
                               ...
                               keepCommonOnly = FALSE, imputeMissing = FALSE, removeDuplicates = FALSE)
{
  ...
  {
    dataList[[i]] = pancreasData[[names(pancreasData)[i]]]
    names(dataList)[i] = pancreasData[i]$title
  }
